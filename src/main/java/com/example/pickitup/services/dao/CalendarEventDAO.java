...
public class CalendarEventDAO {
    ...
    public List<String> getEvents(String startTime) {
        ...
        String sql = "SELECT title, event_description FROM calendar_events WHERE start_time LIKE ?";

        try (Connection connection = DatabaseConnection.connect();
             PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, startTime + "%"); // Match any events starting with the same date
